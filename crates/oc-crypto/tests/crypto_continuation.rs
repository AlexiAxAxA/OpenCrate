...
use core::cell::Cell;
use oc_crypto::{
    AeadAlg, CryptoError, PayloadKey,
    stream::{StreamError, seal_chunks},
};
...
fn xwing_seal_rejects_noncanonical_classical_key_before_rng() {
    ...
    .unwrap();
    let opened = oc_crypto::seal::open_xwing(&secret, &canonical, b"context", b"aad").unwrap();
    ...
    *alias.last_mut().unwrap() |= 0x80;
    ...
    if let Ok(blob) = &result {
        assert!(
            oc_crypto::seal::open_xwing(&secret, blob, b"context", b"aad").is_err(),
            "control did not reproduce mismatch"
        );
    }
...
fn zero_chunk_size_is_rejected_before_callbacks_or_rng() {
    ...
    let result = seal_chunks(
        ...
        |_| {
            ...
            Ok::<_, core::convert::Infallible>(0)
        },
        |_| {
    ...
    assert!(
        matches!(result, Err(StreamError::Crypto(CryptoError::BadLength))),
