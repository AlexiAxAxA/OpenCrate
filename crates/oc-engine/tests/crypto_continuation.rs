...
use oc_crypto::{secret::X25519Secret, seal};
...
fn request() -> PackRequest<'static> {
    PackRequest { original_name: "private.txt", policy: Policy::deny_all(),
        chunk_size: 4096, org_id: b"org".to_vec(), authority_urls: vec![],
        recipient: Recipient::None, coauthors: None }
}

fn keys() -> PublicKeys<'static> {
    let public = seal::x25519_public(&X25519Secret::from_bytes([0x31; 32]));
    PublicKeys { author: [0x42; 32], authority_sealing: public,
        authority_lease_verify: [0x53; 32], device: public,
        device_hardware_hybrid: None, device_hybrid: None, device_tpm: None }
}
...
fn changed_chunk_size_cannot_assemble_a_header_with_a_different_payload_key() {
    ...
    let result = session.assemble(&request, &keys(), SealedInfo {
        total_len: 6, chunk_count: 1, tree_root: [0x64; 32],
    }, &mut Fixed);
    if let Ok(assembled) = &result {
        ...
        let policy = oc_format::header::Header::policy_hash(&assembled.header, &parsed.spans).unwrap();
        let slot = header.key_slots.iter().find_map(|s| match s {
            oc_format::header::KeySlot::Known(s) if s.kind == oc_format::header::SlotKind::AuthorDevice => Some(s),
            _ => None,
        }).unwrap();
        ...
        let opened = seal::open(&secret, &seal::SealedBlob {
            enc: slot.enc.clone(), nonce: slot.nonce, ct: slot.ct.clone(),
        }, &info, &policy).unwrap();
        let (a, b) = opened.split_at(32);
        let kek = oc_crypto::kdf::derive_kek(&header.file_id, &header.org_id,
            &oc_crypto::SecretA::from_bytes(a.try_into().unwrap()),
            &oc_crypto::SecretB::from_bytes(b.try_into().unwrap()));
        let cek = oc_crypto::wrap::unwrap_cek(&kek, &header.wrapped_cek, &slot.commitment, &core).unwrap();
        let reader_key = oc_crypto::kdf::derive_payload_key(&cek, &header.header_salt,
            &header.file_id, header.chunk_size, header.suite.aead);
        assert!(reader_key.expose() != plan.payload_key.expose(), "probe did not reproduce key mismatch");
    }
...
fn pack_request_debug_redacts_private_filename() {
    ...
    assert!(format!("{first:#?}") == format!("{other:#?}"), "private filename affects pretty Debug");
}
...
fn editing_piece_debug_redacts_plaintext_and_nonce_seed() {
    ...
    assert!(format!("{first:#?}") == format!("{second:#?}"), "plaintext or seed affects pretty Debug");
}
